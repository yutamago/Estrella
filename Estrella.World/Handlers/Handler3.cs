...
namespace Estrella.World.Handlers
{
    public sealed class Handler3
    {
        ...
        {
            using (var pack = new Packet(SH3Type.BackToWorldListFromChar))
            {
        ...
        public static void TransferKey(WorldClient client, Packet packet)
        {
            ...
            var transfer = ClientManager.Instance.GetTransfer(key);
            if (transfer != null)
            {
                ...
                if (!client.Host.StartsWith("127.0") && !client.Host.StartsWith("192.") &&
                    transfer.HostIP != client.Host)
                {
                ...
                {
                    if (ClientManager.Instance.RemoveTransfer(transfer.Hash) &&
                        (!Program.Maintenance || transfer.Admin > 0)) //admins can still login
                    {
        ...
        private static void SendCharacterList(WorldClient client)
        {
            ...
            using (var packet = new Packet(SH3Type.CharacterList))
            {
                ...
                foreach (var ch in client.Characters.Values)
                {
