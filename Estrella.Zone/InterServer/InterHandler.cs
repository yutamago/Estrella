...
namespace Estrella.Zone.InterServer
{
    public sealed class InterHandler
    {
        ...
        public static void FunctionAnswer(WorldConnector pConnector, InterPacket pPacket)
        {
            ...
            var result = InterFunctionCallbackProvider.Instance.GetReadFunc(id)(pPacket);
        ...
        public static void HandleTransfer(WorldConnector lc, InterPacket packet)
        {
            ...
            if (v == 0)
            {
                ...
                if (!packet.TryReadInt(out accountid) || !packet.TryReadString(out username) ||
                    !packet.TryReadInt(out CharID) || !packet.TryReadString(out hash) ||
                    !packet.TryReadByte(out admin) || !packet.TryReadString(out hostip))
                {
                    return;
                }

                var ct = new ClientTransfer(accountid, username, CharID, admin, hostip, hash);
        ...
        public static void SendChangeZoneToWorld(ZoneCharacter character, ushort mapid, int x, int y, string ip,
            ushort port, ushort randomid)
        {
        ...
        public static void TransferClient(byte zoneID, ushort mapid, int accountID, string userName, int CharID,
            string charName, ushort randid, byte admin, string hostIP)
        {
