...
namespace Estrella.Zone
{
    ...
    public sealed class CommandHandler
    {
        ...
        {
            var ItemID = ushort.Parse(param[1]);
            var count = byte.Parse(param[2]);
        ...
        private void NpcInfo(ZoneCharacter chr, params string[] param)
        {
            ...
            {
                var Targetnpc = chr.CharacterInTarget as Npc;
        ...
        private void SendMessageChat(ZoneCharacter character, string chat)
        {
            lock (chat)
                ...
                {
                    var ne = character.CharacterInTarget as Npc;
        ...
        {
            var player = param[1];
            var playerC = ClientManager.Instance.GetClientByName(player);
        ...
        {
            var animid = param.Length >= 2 ? byte.Parse(param[1]) : (byte) 50;
        ...
        {
            var lvls = param.Length >= 2 ? byte.Parse(param[1]) : (byte) 1;
        ...
        {
            var exp = param.Length >= 2 ? uint.Parse(param[1]) : 1234567;
            character.GiveExp(exp);
        }

        private void KillAllMobs(ZoneCharacter character, params string[] param)
        {
            var dmg = (ushort) (param.Length >= 2 ? ushort.Parse(param[1]) : 900001);
            foreach (var obj in character.Map.Objects.Values.Where(o => o is Mob))
            {
                var mob = obj as Mob;
        ...
        {
            var other = (ZoneCharacter) character.Question.Object;
            switch (answer)
            {
                case 0:
        ...
        private void CreateMobSpawn(ZoneCharacter character, params string[] param)
        {
            if (param.Length == 2)
            {
                character.Map.MobBreeds.Add(
                    MobBreedLocation.CreateLocationFromPlayer(character, ushort.Parse(param[1])));
            }
        }
            ...
            if (param.Length == 2)
            {
                if (param[1] == "current")
                {
                    Program.DatabaseManager.GetClient().ExecuteQuery(
                        "DELETE FROM `" + Settings.Instance.zoneMysqlDatabase + "`.`Mobspawn` WHERE MapID='" +
                        character.MapID + "'");
                ...
                else if (param[1] == "all")
                {
                    Program.DatabaseManager.GetClient()
                        .ExecuteQuery("DELETE FROM `" + Settings.Instance.zoneMysqlDatabase + "`.`Mobspawn`");
        ...
        private void SpawnMob(ZoneCharacter character, params string[] param)
        {
            ...
            {
                var mob = new Mob(param.Length == 2 ? id : (ushort) 1045, new Vector2(character.Position));
        ...
        private void SpawnMobT(ZoneCharacter character, params string[] param)
        {
            ...
            if (DataProvider.Instance.MobsByID.ContainsKey(id))
            {
                ...
                {
                    var mob = new Mob(id, new Vector2(character.Position));
                    character.Map.FullAddObject(mob);
                }
            }
            else character.DropMessage("Monster ID not found.");
        ...
        private void ChangeMap(ZoneCharacter character, params string[] param)
        {
            ...
            if (param.Length > 2)
            {
                var x = int.Parse(param[2]);
                var y = int.Parse(param[3]);
        ...
        private void WritePacket(ZoneCharacter character, params string[] param)
        {
            if (param.Length >= 3)
            {
                var header = byte.Parse(param[1]);
                var type = byte.Parse(param[2]);
        ...
        private void AdminLevel(ZoneCharacter character, params string[] param)
        {
            ...
            InterHandler.SendWorldMessage(WorldMessageTypes.Level20, "Fun!");
        }
        ...
        public CommandStatus ExecuteCommand(ZoneCharacter character, string[] command)
        {
            ...
            if (_commands.TryGetValue(command[0].ToLower(), out info))
            {
                ...
                {
                    var wholeCommand = string.Join(" ", command);
                    Log.WriteLine(LogLevel.Exception, "Exception while handling command '{0}': {1}", wholeCommand,
                        ex.ToString());
