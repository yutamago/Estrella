...
namespace Estrella.Util.DUpdater
{
        ...
        /* using (MySqlConnection connection = new MySqlConnection(ConnectionStringbuilder.CreateConnectionString(this._setting)))
            {
                ...
                try
                {
                    using (var reader = new MySqlCommand("SELECT [Version] FROM [EstrellaVersion]", connection).ExecuteReader())
                    {
                        if (reader.Read())
                        {
                ...
                if (Directory.Exists("SQL"))
                {
                    ...
                    if (version < patch)
                    {
                        ...
                        using (MySqlCommand cmd = new MySqlCommand("DELETE FROM [EstrellaVersion];", connection))
                            cmd.ExecuteNonQuery();
                        using (MySqlCommand cmd = new MySqlCommand("INSERT INTO [EstrellaVersion] VALUES (" + patch.ToString() + ");", connection))
                            cmd.ExecuteNonQuery();
                    }
                    else
                    {
    ...
    */
        ...
        {
            using (var cmd = new MySqlCommand(pQuery, pConnection))
            {
